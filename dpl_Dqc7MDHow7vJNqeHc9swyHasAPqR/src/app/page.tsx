...
const nav = [["Live Stream","LS"],["Dashboard","DB"],["Recordings","RC"],["AI Analytics","AI"],["Reports","RP"],["Local Files","LF"],["Server Files","SF"],["Settings","ST"]];
...
function Dashboard() {
  ...
  return <div className="app-shell">{msg && <div className="toast">{msg}</div>}
    <aside className="side"><div className="brand"><span className="mark"/><div>Aspire Smart Vision<small>Live Monitoring & Analytics</small></div></div><nav>{nav.map(([x,ic],i)=><button key={x} className={i===0?"nav active":"nav"}><em>{ic}</em>{x}</button>)}</nav><div className="device"><b>Device Status</b><p><Dot ok={connected}/>{connected?"Online":"Checking"}</p><p>Device ID <strong>Smart Helmet Pi</strong></p><p>Room <strong>{ROOM}</strong></p><div className="bar"><span/></div><p>Audio <strong>{audioOn?"Enabled":"Muted"}</strong></p></div></aside>
    <main className="main"><header><div><h1>Live Stream</h1><p>Real-time stream from Camera #CAM-1023</p></div><div className="top"><span>3</span><button>sun</button><b>A</b><strong>Site Manager</strong></div></header>
